...
public class TraceUpdateActivity extends Activity implements LocationSource,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.trace_update_activity);
		...
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (btn.isChecked()) {
					...
					mTraceClient.queryProcessedTrace(2, AMapUtil.parseTraceLocationList(record.getPathline()) , LBSTraceClient.TYPE_AMAP, TraceUpdateActivity.this);
	...
	protected void saveRecord(List<AMapLocation> list, String time) {
		if (list != null && list.size() > 0) {
			...
			Toast.makeText(TraceUpdateActivity.this, "没有记录到路径", Toast.LENGTH_SHORT)
					.show();
		}
	}
...
//		}
	...
	public void record(View view) {
		Intent intent = new Intent(TraceUpdateActivity.this, RecordActivity.class);
		...
		if (lineID == 1) {
			if (linepoints != null && linepoints.size()>0) {
				...
				if (mlocMarker == null) {
					...
					Toast.makeText(TraceUpdateActivity.this, "距离"+mDistance, Toast.LENGTH_SHORT).show();
