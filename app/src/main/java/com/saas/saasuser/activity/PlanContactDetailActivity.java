 ...
 * 安排联络详情
 */
public class PlanContactDetailActivity extends BaseActivity implements NetIntentCallBackListener.INetIntentCallBack {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_plan_contact_detail);
        ButterKnife.bind(this);
        util = new SharedPreferencesUtil(this);
        Util.setHeadTitleMore(this, "安排联络", true);
        btnSubmit.setText("确定");
        orderId = this.getIntent().getStringExtra("orderId");
        //Log.e("订单id7899：", orderId);
        new NetIntent().client_getOrderDetailById(orderId, util.getUserId(), "1", new NetIntentCallBackListener(this));//   TODO   根据id去拿订单数据显示
        //LogUtils.e("请求订单：");
        if (dialog == null) {
    ...
    public Section<FruitCategory, Fruit> getSection() throws JSONException {
        ...
        Fruit fruit3 = new Fruit("司机电话:", StringUtils.repalceEmptyString(json.getString("FPhone").toString()));
        Fruit fruit4 = new Fruit("安排车辆:", StringUtils.repalceEmptyString(json.getString("BOVehicleNum").toString()));
        Fruit fruit5 = new Fruit("车型名称:", StringUtils.repalceEmptyString(json.getString("FieldValue").toString()));
        Fruit fruit6 = new Fruit("车身颜色:", StringUtils.repalceEmptyString(json.getString("VehicleColor").toString()));
        Fruit fruit7 = new Fruit("联络人:", StringUtils.repalceEmptyString(json.getString("BOPlanName").toString()));
        Fruit fruit8 = new Fruit("联络电话:", StringUtils.repalceEmptyString(json.getString("BOPlanPhone").toString()));
    ...
    public Section<FruitCategory, Fruit> getSection1() throws JSONException {
        ...
        Fruit fruit1 = new Fruit("日期：", StringUtils.repalceEmptyString(StringUtils.tripData(json.getString("BOUseDate").toString())));
    ...
    public Section<FruitCategory, Fruit> getSection2() throws JSONException {
        ...
        if (StringUtils.equals("1", json.getString("VehicleType").toString())) {
            strCarModel = "小轿车(5座以下)";
        } else if (StringUtils.equals("2", json.getString("VehicleType").toString())) {
            strCarModel = "商务车(6-8座)";
        } else if (StringUtils.equals("3", json.getString("VehicleType").toString())) {
            strCarModel = "商旅车(9-15座)";
        } else if (StringUtils.equals("4", json.getString("VehicleType").toString())) {
            strCarModel = "中巴车(16-25座)";
        } else if (StringUtils.equals("5", json.getString("VehicleType").toString())) {
            strCarModel = "小型巴士(26-37座)";
        } else if (StringUtils.equals("6", json.getString("VehicleType").toString())) {
        ...
        if (StringUtils.equals("1", json.getString("BOAssignCarLevel").toString())) {
            ...
            strCarLevel = "豪华车型";
        } else if (StringUtils.equals("5", json.getString("BOAssignCarLevel").toString())) {
            strCarLevel = "巴士车型";
        } else {
            strCarLevel = "未选择";
        }
        ...
        Fruit fruit8 = new Fruit("指定车辆等级：", strCarLevel);
    ...
    public void onResponse(String response) {
        ...
        try {
            ...
            ToastUtils.showShortToastSafe(PlanContactDetailActivity.this, jsonObject.getString("ErrMsg"));
    ...
    public void onViewClicked() {
        new NetIntent().client_driverOrUserConfirmOrder(orderId, util.getUserId(), "1", new NetIntentCallBackListener(this));//TODO    开始用车
        //ToastUtils.showShortToastSafe(PlanContactDetailActivity.this, "此功能还没有接口");
        if (dialog == null) {
